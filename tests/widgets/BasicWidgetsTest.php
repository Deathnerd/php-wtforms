...
class BasicWidgetsTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertEquals('<input id="id" name="bar" type="text" value="foo">',
            (new TextInput)->__invoke($this->field));
    }

    public function testPasswordInput()
    {
        $this->assertEquals('<input id="id" name="bar" type="password" value="">',
            (new PasswordInput)->__invoke($this->field));
        $this->assertEquals('<input id="id" name="bar" type="password" value="foo">',
            (new PasswordInput(false))->__invoke($this->field));
    }

    public function testHiddenInput()
    {
        $this->assertEquals('<input id="id" name="bar" type="hidden" value="foo">',
            (new HiddenInput)->__invoke($this->field));
        $this->assertContains('hidden', (new HiddenInput)->field_flags);
    }

    public function testCheckboxInput()
    {
        $this->assertEquals('<input checked id="id" name="bar" type="checkbox" value="v">',
            (new CheckboxInput)->__invoke($this->field, ["value" => "v"]));
    ...
    public function testRadioInput()
    {
        ...
        $field->checked = true;
        $this->assertEquals('<input checked id="" name="f" type="radio" value="foobar">',
            (new RadioInput)->__invoke($field));
    }

    public function testSubmitInput()
    {
        $this->field->label = new Label($this->field->id, $this->field->label);
        $this->assertEquals('<input id="id" name="bar" type="submit" value="label">',
            (new SubmitInput)->__invoke($this->field));
    }
    ...
    public function testOption()
    {
        ...
        $this->assertEquals('<option data-baz-bar="ohai" selected value="foobar">baz</option>',
            (new Option)->__invoke($field, ["data_baz-bar" => "ohai"]));;
    }
