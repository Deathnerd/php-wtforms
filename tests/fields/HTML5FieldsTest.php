...
class HTML5FieldsTest extends \PHPUnit_Framework_TestCase
{
  public function testHTML5Fields()
  {
    ...
    $this->assertEquals('<input id="url" name="url" type="url" value="http://github.com/Deathnerd/php-wtforms">', "$form->url");
    $this->assertEquals('<input id="email" name="email" type="email" value="foo@bar.com">', "$form->email");
    $this->assertEquals('<input id="datetime" name="datetime" type="datetime" value="2013-09-05 00:23:42">', "$form->datetime");
    ...
    $this->assertEquals('<input id="dt_local" name="dt_local" type="datetime-local" value="2013-09-05 00:23:42">', "$form->dt_local");
    ...
    $this->assertEquals('<input id="decimal" name="decimal" step="1" type="number" value="43.5">', "$form->decimal");
    $this->assertEquals(43.5, $form->decimal->data);
    $this->assertEquals('<input id="int_range" name="int_range" step="1" type="range" value="4">', "$form->int_range");
    $this->assertEquals(4, $form->int_range->data);
    $this->assertEquals('<input id="decimal_range" name="decimal_range" step="any" type="range" value="58">', "$form->decimal_range");
